...
#
# Adapted from RLinf (rlinf/models/embodiment/modules/rlt_token_transformer.py).
# Difference from the original: the number of RL tokens is configurable via
# ``num_rl_tokens`` (RLinf hardcodes a single token).
...
class RLTTokenEncoder(nn.Module):
    """Compress VLA prefix embeddings into ``num_rl_tokens`` RL tokens."""
...
class RLTTokenDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Decoder inputs are [rl_tokens, shifted_targets]: at most
        # num_rl_tokens + prefix_seq_len - 1 positions.
        self.decoder_pos_enc = nn.Parameter(
            sinusoidal_pe_init(self.prefix_seq_len + self.num_rl_tokens, self.embed_dim)
        )
        self.layers = nn.ModuleList(
            [
                RLTSelfAttentionLayer(
    ...
    def forward(
        ...
        input_seq_len = decoder_inputs.shape[1]
        decoder_inputs = decoder_inputs + self.decoder_pos_enc[:input_seq_len].to(
        ...
        # Position num_rl_tokens - 1 + i (the last RL token, then teacher-forced
        # target i-1) predicts target i, so the last target_seq_len outputs
        # reconstruct the full target sequence.
        return self.output_proj(x[:, self.num_rl_tokens - 1 :])


class RLTTokenTransformer(nn.Module):
    """RLT encoder with an autoregressive reconstruction objective.

    The ``num_rl_tokens`` RL tokens produce a flattened z_rl feature of
    ``num_rl_tokens * embed_dim``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder = RLTTokenEncoder(num_layers=num_layers, **common_kwargs)
        self.decoder = RLTTokenDecoder(
            num_layers=num_decoder_layers if num_decoder_layers is not None else num_layers,
            **common_kwargs,
        )
