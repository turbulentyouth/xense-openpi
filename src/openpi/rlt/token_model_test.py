...
#
# Adapted from RLinf (tests/unit_tests/test_rlt_token_transformer.py), plus
# coverage for the generalized ``num_rl_tokens`` parameter.
...
def _make_model(*, prefix_seq_len: int = 5, num_rl_tokens: int = 1) -> RLTTokenTransformer:
    torch.manual_seed(0)
...
def test_multiple_rl_tokens_shapes_and_z_dim():
    num_rl_tokens = 3
    model = _make_model(prefix_seq_len=4, num_rl_tokens=num_rl_tokens)
    model.eval()
    prefix_embs = torch.randn(2, 4, model.input_dim)

    rl_tokens = model.encode(prefix_embs)
    assert rl_tokens.shape == (2, num_rl_tokens, model.embed_dim)
    assert model.encode_flat(prefix_embs).shape == (2, model.z_dim)
    assert model.z_dim == num_rl_tokens * model.embed_dim

    reconstructed, _ = model.reconstruct(prefix_embs)
    assert reconstructed.shape == prefix_embs.shape

    loss, info = model.loss(prefix_embs)
    assert loss.ndim == 0
    assert info["z_rl"].shape == (2, model.z_dim)


def test_multiple_rl_tokens_causal_mask_blocks_future_teacher_targets():
    model = _make_model(num_rl_tokens=3)
    model.eval()
    rl_tokens = torch.randn(1, 3, model.embed_dim)
    targets = torch.randn(1, model.prefix_seq_len, model.input_dim)
